		...
		this.createdAt = '';
            ...
            function getOrCreateWishlist() {
                ...
                if (wishlist.id) {
                    ...
                    accPromise.then(function (successAccount) {
                        newWishlist.customerId = successAccount.id;
                    });
                    accPromise.finally(function () {
                        newWishlist.id = "98hw85Y945e6U358";
                        newWishlist.owner = "345MWyh7w4hY98W6";
                        newWishlist.title = "Floyd's Birthday Wishlist";
			newWishlist.description = "Test Bring on the presents:)";
                        newWishlist.createdAt = "2012-07-12T12:42:23.777Z";
			
			var item = {};
			item.product = "yKsU0y+PQUPgTIYm";
			item.amount = "1";
			item.note = "Always wanted a Ferrari";
			item.createdAt = "2012-07-12T13:23:31.415Z";
			newWishlist.items = [item];

                        WishlistREST.Wishlist.all('wishlists').post(newWishlist).then(function (response) {
                            wishlist.id = response.wishlistId;
                            deferredWishlist.resolve({ wishlistId: wishlist.id });
            ...
            function createWishlistItem(product, prices, qty, config) {
                ...
                getOrCreateWishlist().then(function (wishlistResult) {
                    ...
                    WishlistREST.Wishlist.one('wishlists', wishlistResult.wishlistId).all('items').post(item).then(function () {
